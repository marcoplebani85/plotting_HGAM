...
# GI: models with a global smoother for all observations, plus group-level smoothers, the wiggliness of which is estimated separately 
start_time <- Sys.time()
gam_GI1 <- mgcv::bam(density ~ Taxon # main effect
...
end_time <- Sys.time()
end_time - start_time # it takes about 2.2 minutes to run on my 2014 mac mini
# gam.check(gam_GI1)
# k.check(gam_GI1)
MuMIn::AICc(gam_GI1)
...
pred <- predict(object= gam_GI1, newdata=nn, type="response", se.fit=T)
nn$fit <- pred$fit
nn$se <- pred$se.fit
